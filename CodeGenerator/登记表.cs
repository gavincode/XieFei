...
namespace Excel.Export.AutoCode
{
    public class 登记表
    {
        ...
        public class 登记簿2
        {
            ...
            public static Row Row4 = new Row(sheetName, 4, 1);
            public static Row Row23 = new Row(sheetName, 23, 1);
        }
        ...
        public class Row
        {
            ...
            public void Fill<T>(IEnumerable<T> dataList, Func<T, List<Object>> setAction)
            {
                ...
                foreach (var data in dataList)
                {
                    ...
                    for (int j = BeginColum; j < itemVaues.Count; j++)
                    {
                        var cell = Workbook.GetSheet(SheetName).GetRow(beginIndex).GetCell(j);

                        SetCellValue(cell, itemVaues[j]);
                    }
                }
            }
        }
